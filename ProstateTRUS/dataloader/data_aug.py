...
class RandomRotateTransform(object):
    ...
    def __call__(self, sample):
        ...
        if np.random.uniform() < self.p_per_sample:
            ...
            volume = rotate(volume, angle=rand_angle, axes=(-2, -3), reshape=False, order=1)
        sample[self.volume_key] = volume
...
class resample(object):
    def __init__(self, volume_key='volume', label_key='label', image_size=[96, 64, 96]):
        self.label_key = label_key
        self.volume_key = volume_key
    ...
    def __call__(self, sample):
        ...
        resample1.SetOutputOrigin(sample['origin'])
        resample1.SetOutputDirection(sample['direction'])
        resample1.SetSize(self.image_size)
        resample1.SetOutputSpacing(self.image_size)
        new_volume = resample1.Execute(sitk.GetImageFromArray(volume))

        resample2.SetOutputOrigin(sample['origin'])
        resample2.SetOutputDirection(sample['direction'])
        resample2.SetSize(self.image_size)
        resample2.SetOutputSpacing(self.image_size)
        new_label = resample2.Execute(sitk.GetImageFromArray(label))
        sample['volume'], sample['label'] = sitk.GetArrayFromImage(new_volume), sitk.GetArrayFromImage(new_label)
        return sample
